...
def test_6_login_as_approver(page):
    s_page = ProcurementLoginPage(page)
    s_page.perform_login(
...
def test_7_approver_requisition_approve(page):
    d_page = DashboardPage(page)
    d_page.goto_procurement()
    p_page = ProcurementHomePage(page)
...
def test_8_login_as_initiator(page):
    s_page = ProcurementLoginPage(page)
    s_page.perform_login(
        user_name=proc_user,
        pass_word=proc_pass
...
def test_9_initiator_find_approver_2_of_the_requisition(page):
    d_page = DashboardPage(page)
    d_page.goto_procurement()
    p_page = ProcurementHomePage(page)
...
def test_10_test_6_login_as_approver_2(page):
    s_page = ProcurementLoginPage(page)
    s_page.perform_login(
...
def test_11_approver_2_requisition_approve(page):
    d_page = DashboardPage(page)
    d_page.goto_procurement()
    p_page = ProcurementHomePage(page)
    p_page.navigate_to_requisition_approve_list()
    p_page.get_full_page_screenshot('full_page_screenshot_14')
    r_page = RequisitionApproveList(page)
    r_page.search_requisition(req_num)
    r_page.select_requisition()
    r_page.approve_requisition()
...
def test_12_login_as_initiator_for_check_requisition_status(page):
    s_page = ProcurementLoginPage(page)
    s_page.perform_login(
        user_name=proc_user,
        pass_word=proc_pass
...
def test_13_check_requisition_approved(page):
    d_page = DashboardPage(page)
    d_page.goto_procurement()
    p_page = ProcurementHomePage(page)
    p_page.navigate_to_requisition_list()
    # p_page.get_full_page_screenshot('full_page_screenshot_17')
    r_page = RequisitionList(page)
    r_page.search_requisition(req_num)
