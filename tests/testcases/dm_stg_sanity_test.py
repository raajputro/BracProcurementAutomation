...
def test_5_login_as_2nd_approver_and_approve_requisition(page):
    ...
    proc_login_page.perform_login(
        ...
        pass_word=proj_pass,
        timeout=90000  # Increased timeout for login
    )
    proc_dashboard_page = DashboardPage(page)
    proc_dashboard_page.goto_procurement()
...
def test_16_marketplace_item_receive_bill_creation_and_submit_in_procurement(page):
...
def test_17_vendor_bill_recommender1_approval_process_in_procurement(page, new_tab):
...
def test_18_vendor_bill_recommender2_approval_process_in_procurement(page, new_tab):
    ...
    global bill_approver_id3
    bill_approver_id3 = str(int(bill_list_page.find_approver_id(bill_num)))
    print(f"Bill Approver : {bill_approver_id3}")
...
def test_19_vendor_bill_approver_approval_process_in_procurement(page, new_tab):
    print("Test 19: Vendor bill approver approval...")
    proc_login_page = ProcurementLoginPage(page)
    proc_login_page.perform_login(
        given_url=proj_url,
        user_name=bill_approver_id3,
        pass_word=proj_pass,
        timeout=60000
    )
