...
# === Read pytest_html_report.yml so conftest is reusable ==============
...
# =========================
# HRMS summary + donut chart injection into pytest-html-report (CLEAN)
# =========================
def pytest_runtest_logreport(report: pytest.TestReport):
    ...
    if report.passed:
        HRMS_METRICS["passed"] += 1
    elif report.failed:
        HRMS_METRICS["failed"] += 1
    elif report.skipped:
        HRMS_METRICS["skipped"] += 1

    HRMS_METRICS["durations"].append(getattr(report, "duration", 0.0))
...
def pytest_sessionfinish(session: pytest.Session, exitstatus: int):
    ...
    start_token = "<!-- HRMS SUMMARY START -->"
    end_token = "<!-- HRMS SUMMARY END -->"
    if start_token in html and end_token in html:
        ...
        # ---- 2) Compute metrics ----
    passed = HRMS_METRICS["passed"]
    failed = HRMS_METRICS["failed"]
    skipped = HRMS_METRICS["skipped"]

    total_tests = passed + failed + skipped
    if total_tests == 0:
        total_tests = 1

    total_duration = sum(HRMS_METRICS["durations"])
    ...
    <div style="max-width:1200px;margin:0 auto;padding:0 24px;">
      ...
      <div style="margin-top:32px;text-align:center;">
        ...
        <div style="width:200px;height:200px;margin:0 auto;">
          <canvas id="hrmsStatusChart"
                  style="width:200px!important;height:200px!important;max-width:200px;max-height:220px;">
          </canvas>
        </div>
        <!-- counts + % legend like your screenshot 3 -->
        <div id="hrmsStatusLegend" style="margin-top:12px;text-align:center;font-size:13px;color:#374151;"></div>
      </div>
...
<script>
  (function() {{
    ...
    function buildChart() {{
      var canvas = document.getElementById('hrmsStatusChart');
      if (!canvas) return;
      ...
      var legend = document.getElementById('hrmsStatusLegend');
      if (legend) {{
        legend.innerHTML =
    ...
    function hideOldHeader() {{
      var summaryRoot = document.getElementById('hrms-summary-root');
...
"""
    ...
    if tr:
        tr.write_line(f"Updated HTML report with HRMS summary + chart: {latest}")
